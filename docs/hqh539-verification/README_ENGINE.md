...
## Digest profiles (REF / P32 / LEGACY)

See **[DUAL_PROFILES.md](DUAL_PROFILES.md)**.

- **REF** (`golden_vectors.json`): binds to `hqh539.py` — canonical KATs.
- **P32** (`golden_vectors_p32.json`): fixed 32-byte finalize (deploy/demo style).
- **LEGACY** (`golden_vectors_legacy_orphaned.json`): archived; do not use.
...
- Expanded KATs: lengths, binary, salt sensitivity, T3 integrality
- AE roundtrip: HQH KDF + ChaCha20-Poly1305 + `.hqh539enc` package
- RTL vectors: `rtl_vectors/` (T4121 product profile — see DUAL_PROFILES.md)
