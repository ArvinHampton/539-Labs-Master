...
- **Architecture A / Bott on \(\mathcal{O}_{\mathrm{res}}\)** (`Architecture_A_Ores_Programme.md`, A1–A2 notes, `scripts/architecture_A1_A2_probe.py`): \(\beta_\sharp\) seed-independent fibers \((68^3,67^5)\); simplicial lift \(E(\mathcal{O}_{\mathrm{res}})\to E(X_{\mathrm{disc}})\); A3–A5 geometric \(KO\) open; provenance — residual flux quanta under (S), not free \(T^\sharp\); Option 3 for free dynamics
