...
- **Architecture A / Bott on \(\mathcal{O}_{\mathrm{res}}\)** (`Architecture_A_Ores_Programme.md`, `Architecture_A_Spin_KO_Draft.md`, `scripts/architecture_A_ores_probe.py`): discrete classifying map \(f\) injective; Bott-clock fibers; provenance mandatory — residual flux quanta under (S), not free \(T^\sharp\); no No-Go lift
