...
- **Non-circular 539-object set** (`Object539_NonCircular_Construction.md`, `scripts/verify_object539_construction.py`): residual democratic charge-sector cores \(\mathcal{O}_{\mathrm{res}}\) with \(\lvert\mathcal{O}_{\mathrm{res}}\rvert=\lfloor(N_{\mathrm{flux}}-f_{\max})/9\rfloor=539\); free-dynamics basins still 2; 1001 provenance (`Provenance_1001_Shells.md`); E1–E2 probes (`scripts/probe_E1E2_bott_words.py`)
- **Architecture A / Bott on \(\mathcal{O}_{\mathrm{res}}\)** (`Architecture_A_Ores_Programme.md`, A1–A3 notes, probes): \(\beta_\sharp\) fibers; simplicial lift; continuous \(\Phi\colon\lvert E(\mathcal{O}_{\mathrm{res}})\rvert\to\mathrm{Gr}_1(V)\hookrightarrow BO\) (`Architecture_A3_Geometric_BO_BSpin.md`); A4–A5 \(B\mathrm{Spin}\)/\(KO\) open; provenance — residual flux under (S); Option 3 for free dynamics
- **HQH-539 security reductions (exploration only)** (`HQH539_Security_Reductions_Exploration.md`): Category B framing only; intended goals; packaging/\(\mathcal{O}_{\mathrm{res}}\)/T3 ingredients; **no** completed reduction; T4121 set aside; confidential details not expanded
- **9 Maths pattern exploration** (`Nine_Maths_Pattern_Exploration.md`): Hopf / Faddeev–Niemi / Cartan-torsion vs TTC–RTTC (strongest), RNT and BMMT/HMT (secondary); continuum claims Category B; Option 3 / No-Go intact
- **Discrete patterns on residual carrier (primary pattern dir.)** (`Discrete_Patterns_Residual_Carrier.md`, `scripts/discrete_patterns_residual_probe.py`): core linking \(L_{ab}=B'\mathrm{sgn}(a-b)\); tower coboundary; flat mod-9; RNT substrate — residual-flux provenance only
...
| \(\sigma=N_{\mathrm{HQCC}}\) | \(539\pm 1\) | Crypto: hard budget \(:=L_{\mathrm{pack}}\) under (S); free objects still open (Option 3); **not** from ACE |
