...
- **Generation Individuality** (`Individuality_from_Axiom0_Expansion.tex`, `\input` after Math 1 / TTC): restores \(e/\mu/\tau\) labels from Axiom 0 alone via composite dictionary \((w_j,k_j,c_j)\) — no second continuous axiom
...
- **Photon-ring critical curve** (`PhotonRing_CriticalCurve_Derivation.tex` / `.md`): integer tower → \(f_{\mathrm{snap}}\) → Schwarzschild \(\Delta b_c/b_c\) → \(\Delta r_{\mathrm{ring}}/(GM/c^2)=243/4880\)
...
- **No-Go Theorem (canonical)** (`NoGo_Theorem_Canonical.md` / `.tex`): conclusions (a)(b)(c) hold; programme claims (a′)(b′)(c′) blocked; derives \(N_\star=14\neq539\); empirical 539.9 as hypothesis only
...
- **Empirical phase-locking** (`Empirical_PhaseLocking_Protocol.md`): periodogram/multitaper/Lomb–Scargle + bootstrap free of 539.9
...
- **Bott–HQCC link research** (`Bott_HQCC_Link_Research.md`, `scripts/bott_hqcc_probe.py`): architectures A–D; arithmetic \(539=8\cdot67+3\)
- **Phase 0 executed** (`Phase0_Space_C_Definition.md`, `Phase0_Execution_Report.md`, `scripts/phase0_C2_components.py`): C2 → 2 components ≠ 539
...
- **Non-circular \(L_\star\)** (`NonCircular_Lstar_Exploration.md`, `NonCircular_18_plus_521.md`, `scripts/noncircular_Lstar_probe.py`): \(\lfloor e^3/\ln 3\rfloor=18\) clean; \(18+(4880//9-21)=539\) strongest composite (521 ansatz); endpoints ≠ 539 objects
